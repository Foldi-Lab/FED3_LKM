...
for folder in sorted(os.listdir(import_location)):
    if folder.startswith('FED'):
                ...
                #-----        
                        ...
                        index = 0
                        poke_index = 0
                        # print(len(seconds_elapsed), len(poke_time2), len(seconds_block))
                        # print(seconds_elapsed, seconds_block)
                        for i in range(0, len(poke_time2)):
                            # print(index)
                            # print(seconds_block[index])
                        ...
                        # Create win-stay and lose-shift data
                        ...
                        # print(win_stay_all, win_shift_all, active_poke[-1], lose_shift_all, lose_stay_all, inactive_poke[-1])
                        ...
                        # print('WIN STAY ALL = ', win_stay_proportion_all, '   LOSE SHIFT ALL = ', lose_shift_proportion_all)
                        ...
                        # print(win_stay_comp, win_shift_comp, active_poke_comp[-1], lose_shift_comp, lose_stay_comp, inactive_poke_comp[-1])
                        ...
                        # print('WIN STAY COMPLETED = ', win_stay_proportion_comp, '   LOSE SHIFT COMPLETED = ', lose_shift_proportion_comp)
                        ...
                        if export_results == 'Y':
                            ...
                            # If using time bins get the bin numbers - NOTE: Blocks are NOT collapsed into time bins, the only new data is which time bin each block was completed in    
                                ...
                                results_blocked_w = {'Time (sec from start)': seconds_block, 'Time (sec within block)': seconds_block_within, 'Duration': duration_block, 'Time Bin': bin_num, 'Session Type': session_block, 'Active port': active_block, 
                                ...
                                export_file_blocked_w = pd.DataFrame(results_blocked_w, columns = ['Time (sec from start)', 'Time (sec within block)', 'Duration', 'Time Bin', 'Session Type', 'Active port', 
                                                                                                    'Left Poke', 'Right Poke', 'Active Poke', 'Inactive Poke', 'Total Poke', 'Pellet Count', '% Active Pokes'])
